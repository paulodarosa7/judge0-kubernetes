...
def gerar_casos(qtd):
    """
    Gera pares stdin/expected_output.

    O codigo submetido soma dois inteiros.
    Cada par entrada+saida representa 1 caso de teste.
    """
    casos = []
...
CODIGO_CORRETO = """a, b = map(int, input().split())
print(a + b)
"""
...
def construir_jobs(alunos, tentativas, casos):
    casos_base = gerar_casos(casos)
    jobs = []
...
def main():
    ...
    jobs = construir_jobs(args.alunos, args.tentativas, args.casos)
